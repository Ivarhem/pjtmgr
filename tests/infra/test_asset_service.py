...
def test_create_asset_requires_existing_project(db_session, admin_role_id) -> None:
    admin = _make_admin_user(db_session, admin_role_id)
...
def test_create_asset_rejects_duplicate_name_in_same_project(
    ...
    payload = AssetCreate(
        project_id=project.id, asset_name="APP-01", asset_type="server"
    )
