...
def _make_partner(db_session, name: str = "Import고객사") -> Partner:
...
def test_parse_normal_rows() -> None:
    ...
    result = parse_inventory_sheet(xlsx, partner_id=1)
...
def test_import_creates_assets(db_session, admin_role_id) -> None:
    ...
    xlsx = _build_inventory_xlsx(rows)
    parsed = parse_inventory_sheet(xlsx, partner.id)
    assert len(parsed["errors"]) == 0

    result = import_inventory(db_session, partner.id, parsed["rows"], admin)
    assert result["created"] == 2
    assert result["skipped"] == 0

    # DB 검증
    assets = db_session.query(Asset).filter(Asset.partner_id == partner.id).all()
    assert len(assets) == 2
...
def test_import_skip_existing(db_session, admin_role_id) -> None:
    ...
    # 기존 자산 생성
    existing = Asset(partner_id=partner.id, asset_name="SVR-EXIST", asset_type="server")
    ...
    result = import_inventory(db_session, partner.id, parsed["rows"], admin, on_duplicate="skip")
    assert result["created"] == 1
    assert result["skipped"] == 1


def test_import_nonexistent_partner(db_session, admin_role_id) -> None:
    """존재하지 않는 거래처 Import 시 에러."""
    admin = _make_admin_user(db_session, admin_role_id)
    result = import_inventory(db_session, 99999, [{"asset_name": "X", "partner_id": 99999}], admin)
    assert len(result["errors"]) == 1
    assert result["error_details"][0]["code"] == "partner_not_found"
