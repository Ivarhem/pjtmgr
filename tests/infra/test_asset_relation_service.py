...
def test_create_and_list_relations(db_session, admin_role_id) -> None:
    ...
    proj = create_project(db_session, ProjectCreate(project_code="REL-01", project_name="Rel Test"), admin)
    svr = create_asset(db_session, AssetCreate(project_id=proj.id, asset_name="SVR-R1", asset_type="server"), admin)
    db = create_asset(db_session, AssetCreate(project_id=proj.id, asset_name="DB-R1", asset_type="server"), admin)

    rel = create_asset_relation(db_session, AssetRelationCreate(src_asset_id=svr.id, dst_asset_id=db.id, relation_type="HOSTS"), admin)
    ...
    # 역방향 조회
    rels2 = list_by_asset(db_session, db.id)
    assert len(rels2) == 1
...
def test_self_relation_rejected(db_session, admin_role_id) -> None:
    admin = _make_admin(db_session, admin_role_id)
    proj = create_project(db_session, ProjectCreate(project_code="REL-02", project_name="Self Test"), admin)
    svr = create_asset(db_session, AssetCreate(project_id=proj.id, asset_name="SVR-R2", asset_type="server"), admin)
...
def test_delete_relation(db_session, admin_role_id) -> None:
    admin = _make_admin(db_session, admin_role_id)
    proj = create_project(db_session, ProjectCreate(project_code="REL-03", project_name="Del Test"), admin)
    svr = create_asset(db_session, AssetCreate(project_id=proj.id, asset_name="SVR-R3", asset_type="server"), admin)
    fw = create_asset(db_session, AssetCreate(project_id=proj.id, asset_name="FW-R3", asset_type="security"), admin)
...
def test_update_relation(db_session, admin_role_id) -> None:
    admin = _make_admin(db_session, admin_role_id)
    proj = create_project(db_session, ProjectCreate(project_code="REL-04", project_name="Upd Test"), admin)
    svr = create_asset(db_session, AssetCreate(project_id=proj.id, asset_name="SVR-R4", asset_type="server"), admin)
    app = create_asset(db_session, AssetCreate(project_id=proj.id, asset_name="APP-R4", asset_type="server"), admin)
