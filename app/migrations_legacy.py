...
main.py에서 분리된 레거시 마이그레이션 로직.
Alembic 정식 도입 시 이 모듈은 제거 예정.
"""
import logging
import re
...
def run_migrations() -> None:
    ...
    with engine.connect() as conn:
        ...
        # deal_periods에 customer_id 컬럼 추가 (Period별 매출처)
        for tbl_name in ("deal_periods", "contract_periods"):
            if tbl_name in table_names:
                dp_cols = _table_columns(inspector, tbl_name)
        ...
        # contracts 테이블: deal_* → contract_* 컬럼 리네임
        ...
        # receipt_matches: deal_id → contract_id
        ...
        # 1) customer_contact_roles 테이블 생성
        ...
        # ── contract_periods: is_planned 컬럼 추가 ──
        if "contract_periods" in table_names:
            cp_cols = _table_columns(inspector, "contract_periods")
            if "is_planned" not in cp_cols:
                conn.execute(text(
                    "ALTER TABLE contract_periods ADD COLUMN is_planned BOOLEAN DEFAULT 1 NOT NULL"
                ))
                conn.commit()
                logger.info("마이그레이션: contract_periods.is_planned 컬럼 추가")
                inspector = inspect(engine)

        # ── 성능 인덱스 추가 ──────────────────────────────────────
        _INDEX_DEFS = [
            ("ix_receipts_revenue_month", "receipts", "revenue_month"),
            ("ix_receipts_receipt_date", "receipts", "receipt_date"),
            ("ix_contracts_status", "contracts", "status"),
            ("ix_contracts_contract_type", "contracts", "contract_type"),
            ("ix_contract_periods_start_month", "contract_periods", "start_month"),
            ("ix_contract_periods_end_month", "contract_periods", "end_month"),
            ("ix_monthly_forecasts_forecast_month", "monthly_forecasts", "forecast_month"),
            ("ix_monthly_forecasts_is_current", "monthly_forecasts", "is_current"),
        ]
        for ix_name, tbl, col in _INDEX_DEFS:
            if tbl in table_names:
                conn.execute(text(
                    f"CREATE INDEX IF NOT EXISTS {ix_name} ON {tbl}({col})"
                ))
        conn.commit()
