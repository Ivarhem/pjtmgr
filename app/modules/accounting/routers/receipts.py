...
@router.post("/contracts/{contract_id}/receipts", status_code=201, dependencies=[require_module_access("accounting", "full")])
def create_receipt(
    contract_id: int,
    data: ReceiptCreate,
    db: Session = Depends(get_db),
...
@router.patch("/receipts/{receipt_id}", dependencies=[require_module_access("accounting", "full")])
def update_receipt(
    receipt_id: int,
    data: ReceiptUpdate,
    db: Session = Depends(get_db),
...
def delete_receipt(
    ...
    svc.delete_receipt(db, receipt_id, current_user=current_user)
