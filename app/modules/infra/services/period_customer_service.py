...
def list_by_period(db: Session, contract_period_id: int) -> list[dict]:
    links = list(
        db.scalars(
            select(PeriodCustomer)
            .where(PeriodCustomer.contract_period_id == contract_period_id)
            .order_by(PeriodCustomer.id.asc())
        )
    )
...
def create_period_customer(
    db: Session, payload: PeriodCustomerCreate, current_user
) -> PeriodCustomer:
    _require_edit(current_user)
    _ensure_period(db, payload.contract_period_id)
    _ensure_customer(db, payload.customer_id)
    _ensure_unique(db, payload.contract_period_id, payload.customer_id, payload.role)

    pc = PeriodCustomer(**payload.model_dump())
    db.add(pc)
    db.commit()
    db.refresh(pc)
    return pc
...
def update_period_customer(
    db: Session, link_id: int, payload: PeriodCustomerUpdate, current_user
) -> PeriodCustomer:
    _require_edit(current_user)
    pc = _get(db, link_id)
...
def delete_period_customer(db: Session, link_id: int, current_user) -> None:
    _require_edit(current_user)
    pc = _get(db, link_id)
    db.delete(pc)
    db.commit()
...
def _get(db: Session, link_id: int) -> PeriodCustomer:
    pc = db.get(PeriodCustomer, link_id)
    if pc is None:
...
def _ensure_unique(
    db: Session, contract_period_id: int, customer_id: int, role: str
) -> None:
    existing = db.scalar(
        select(PeriodCustomer).where(
            PeriodCustomer.contract_period_id == contract_period_id,
            PeriodCustomer.customer_id == customer_id,
            PeriodCustomer.role == role,
        )
    )
    if existing:
        raise DuplicateError("This customer-role is already linked to the period")
...
def _enrich(db: Session, links: list[PeriodCustomer]) -> list[dict]:
    if not links:
        return []
    ...
    result = []
    for l in links:
        d = {c.key: getattr(l, c.key) for c in PeriodCustomer.__table__.columns}
