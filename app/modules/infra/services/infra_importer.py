...
def parse_inventory_sheet(file_bytes: bytes, project_id: int) -> dict:
    ...
    for row in ws.iter_rows(min_row=4, values_only=False):
        ...
        # 필드 추출
        asset_data: dict = {"project_id": project_id}
        row_errors: list[str] = []
...
def import_inventory(
    ...
    if project is None:
        _append_error(errors, error_details, f"프로젝트(ID={project_id})를 찾을 수 없습니다.", code="project_not_found")
        ...
        if existing:
            if on_duplicate == "skip":
                ...
                for field, value in row_data.items():
                    if field in ("project_id",) or value is None:
                        continue
...
def parse_subnet_sheet(file_bytes: bytes, project_id: int) -> dict:
...
def import_subnets(db: Session, project_id: int, parsed_rows: list[dict], current_user, on_duplicate: str = "skip") -> dict:
    ...
    existing = {s.subnet: s for s in db.scalars(select(IpSubnet).where(IpSubnet.project_id == project_id))}
...
def parse_portmap_sheet(file_bytes: bytes, project_id: int) -> dict:
...
def import_portmaps(db: Session, project_id: int, parsed_rows: list[dict], current_user) -> dict:
