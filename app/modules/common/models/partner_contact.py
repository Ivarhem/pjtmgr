"""거래처 담당자 (PartnerContact) - 거래처별 N명의 담당자 관리

담당자 1명이 여러 역할(영업/세금계산서/업무)을 겸할 수 있으며,
역할 정보는 PartnerContactRole 테이블에서 관리한다.
"""
...
class PartnerContact(TimestampMixin, Base):
    ...
    # 레거시 컬럼 — DB NOT NULL 제약 유지용 (실제 역할은 PartnerContactRole로 관리)
    ...
    roles: Mapped[list["PartnerContactRole"]] = relationship(
