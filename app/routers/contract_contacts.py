...
def list_contract_contacts(contract_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
...
def create_contract_contact_compat(contract_id: int, data: ContractContactCreate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
...
def list_period_contacts(period_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
...
def create_period_contact(period_id: int, data: ContractContactCreate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    check_period_access(db, period_id, current_user)
    return svc.create_contract_contact(db, period_id, data)


@router.get("/customers/{customer_id}/contract-contacts")
def list_customer_contract_contacts(customer_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
...
def list_customer_contract_contacts_pivoted(customer_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    return svc.list_by_customer_pivoted(db, customer_id, current_user=current_user)


@router.patch("/contract-contacts/{contact_id}")
def update_contract_contact(contact_id: int, data: ContractContactUpdate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
...
def delete_contract_contact(contact_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
