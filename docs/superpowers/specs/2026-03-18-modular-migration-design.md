...
- **projmgr**: 영업부서 매입매출 관리 서비스. 파일럿 운영 중 (v0.3). SQLite, 단일 앱 구조.
...
- 프로젝트 이름은 `projmgr` 유지 (추후 변경 가능)
- 기존 projmgr는 복사본이므로 제자리 리팩터링 진행

### 파일럿 데이터 처리

기존 projmgr/inframgr의 파일럿 데이터는 **폐기**한다. 통합 후 PostgreSQL에서 새로 시작한다. 이유:
...
- 기존 projmgr는 복사본이므로 최악의 경우 원본에서 다시 시작할 수 있다.
...
- CustomerContactRole 테이블 유지 (projmgr의 역할 모델이 더 성숙)
...
- `base.css`, `components.css`, `utils.js`: 두 프로젝트 것을 병합. projmgr 기준, inframgr 전용 부분만 추가.
