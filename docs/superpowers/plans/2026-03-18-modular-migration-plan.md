...
```python
# 변경 사항:
DATABASE_URL: str = "postgresql://projmgr:projmgr@localhost:5432/projmgr"  # 기본값 변경
...
services:
  db:
    ...
    environment:
      POSTGRES_DB: projmgr
      POSTGRES_USER: projmgr
      POSTGRES_PASSWORD: ${DB_PASSWORD:-projmgr}
    ports:
      - "5432:5432"
    volumes:
    ...
    healthcheck:
      test: ["CMD-SHELL", "pg_isready -U projmgr"]
      interval: 5s
      timeout: 3s
      retries: 5
...
```
APP_ENV=development
DATABASE_URL=postgresql://projmgr:projmgr@db:5432/projmgr
...
기존 SQLite in-memory 세션을 PostgreSQL 테스트 DB로 변경. 테스트용 DB URL은 환경변수 `TEST_DATABASE_URL`로 주입하거나, 기본값으로 `postgresql://projmgr:projmgr@localhost:5432/projmgr_test`를 사용.
...
기존 db 서비스의 init 스크립트로 `projmgr_test` DB를 함께 생성하도록 설정.
...
두 프로젝트의 `utils.js` 비교. 공통 유틸(`apiFetch`, `fmtNumber`, `fmtDate`, `showToast`, `confirmDelete`)은 이미 projmgr의 utils.js에 존재할 가능성 높음. inframgr 전용 유틸이 있으면 추가.
...
- fixture를 projmgr의 conftest 패턴에 맞춤
