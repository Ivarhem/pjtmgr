...
const columnDefs = [
  ...
  },
  {
    ...
    editable: false,
    cellClass: (p) => getGridCellClass("asset_type", p.data),
  },
...
const _classificationLeafOptionsCache = new Map();
let _currentClassificationSchemeMeta = null;
...
function getLastConfiguredClassificationAlias(scheme = _currentClassificationSchemeMeta) {
  if (!scheme) return CLASSIFICATION_LEVEL_ALIAS_DEFAULTS[2];
  const aliases = [
    scheme.level_1_alias,
    scheme.level_2_alias,
    scheme.level_3_alias,
    scheme.level_4_alias,
    scheme.level_5_alias,
  ].filter(Boolean);
  return aliases[aliases.length - 1] || CLASSIFICATION_LEVEL_ALIAS_DEFAULTS[2];
}

function getClassificationManagementUrl() {
  return getCtxProjectId() ? "/classification-schemes" : "/product-catalog";
}

async function resolveActiveClassificationScheme(projectId) {
  if (projectId) {
    const projectSchemes = await apiFetch(`/api/v1/classification-schemes?scope_type=project&project_id=${projectId}`);
    if (projectSchemes.length) return projectSchemes[0];
  }
  const globalSchemes = await apiFetch("/api/v1/classification-schemes?scope_type=global");
  return globalSchemes[0] || null;
}

async function loadClassificationLeafOptions(projectId) {
  const scheme = await resolveActiveClassificationScheme(projectId);
  _currentClassificationSchemeMeta = scheme;
  if (!scheme) return { scheme: null, options: [] };
  if (_classificationLeafOptionsCache.has(scheme.id)) {
    return { scheme, options: _classificationLeafOptionsCache.get(scheme.id) };
  }
  ...
  const parentIds = new Set(activeNodes.filter((node) => node.parent_id).map((node) => node.parent_id));
  const leafOptions = activeNodes
    ...
    .map((node) => ({
      ...
      path_label: node.path_label || node.node_name,
      display_label: node.path_label && node.path_label !== node.node_name
        ? `${node.node_name} (${node.path_label})`
        : node.node_name,
    }))
    .sort((a, b) => a.path_label.localeCompare(b.path_label, "ko-KR"));
  _classificationLeafOptionsCache.set(scheme.id, leafOptions);
  return { scheme, options: leafOptions };
}

function setClassificationSelectOptions(select, options, selectedId = null, emptyText = "-- 선택 안함 --", disabled = false) {
  if (!select) return;
  select.textContent = "";
  ...
  options.forEach((option) => {
    ...
    if (selectedId != null && String(option.id) === String(selectedId)) opt.selected = true;
    select.appendChild(opt);
  });
  select.disabled = !!disabled;
}

function updateClassificationHint({ nodeId = null, scheme = _currentClassificationSchemeMeta, options = null, target = "create" } = {}) {
  const hintId = target === "detail" ? "asset-detail-classification-hint" : "asset-classification-hint";
  const labelId = target === "detail" ? "asset-detail-classification-label" : "asset-classification-label";
  const hint = document.getElementById(hintId);
  const label = document.getElementById(labelId);
  const finalAlias = getLastConfiguredClassificationAlias(scheme);
  if (label) label.textContent = finalAlias;
  if (!hint) return;
  const optionSet = options || (_currentClassificationSchemeMeta ? _classificationLeafOptionsCache.get(_currentClassificationSchemeMeta.id) : []) || [];
  const selected = nodeId ? optionSet.find((item) => String(item.id) === String(nodeId)) : null;
  if (selected) {
    hint.textContent = `선택 경로: ${selected.path_label}`;
    return;
  }
  if (scheme) {
    hint.textContent = `최종 ${finalAlias}만 선택하면 상위 경로는 자동으로 연결됩니다.`;
    return;
  }
  hint.textContent = "사용 가능한 분류가 없습니다. 카탈로그 화면에서 기준 트리를 먼저 등록하세요.";
}
...
async function loadClassificationLevelAliases() {
  ...
  try {
    ...
    if (scheme) {
      _classificationLevelAliases = [
        scheme.level_1_alias || CLASSIFICATION_LEVEL_ALIAS_DEFAULTS[0],
        scheme.level_2_alias || CLASSIFICATION_LEVEL_ALIAS_DEFAULTS[1],
        scheme.level_3_alias || CLASSIFICATION_LEVEL_ALIAS_DEFAULTS[2],
        scheme.level_4_alias || CLASSIFICATION_LEVEL_ALIAS_DEFAULTS[3],
        scheme.level_5_alias || CLASSIFICATION_LEVEL_ALIAS_DEFAULTS[4],
      ];
    }
  } catch (err) {
...
const DETAIL_EDIT_FIELDS = {
  overview: [
    ...
    ["자산명", "asset_name"],
    ["귀속사업", "period_id"],
    ["__CLASSIFICATION__", "classification_node_id"],
    ["제조사", "vendor"],
    ["모델", "model"],
    ["시리얼", "serial_no"],
    ["장비 ID", "equipment_id"],
...
function renderStructuredDetailTab(tab, container) {
  ...
  sections.forEach((sectionConfig) => {
    ...
    visibleFields.forEach(([label, key, fmt]) => {
      ...
      dt.textContent = label === "__CLASSIFICATION__" ? getLastConfiguredClassificationAlias() : label;
...
async function buildDetailEditFields(tab) {
  ...
  for (const [label, key] of fields) {
    ...
    if (!["note", "location", "service_name", "classification_node_id"].includes(key)) {
      fieldWrap.classList.remove("full-width");
    }
    fieldWrap.textContent = label === "__CLASSIFICATION__" ? getLastConfiguredClassificationAlias() : label;
    ...
    if (key === "period_id") {
      ...
      input.addEventListener("change", async () => {
        const classificationSelect = container.querySelector('[data-field="classification_node_id"]');
        if (!classificationSelect) return;
        const projectId = input.value ? Number(input.value) : (getCtxProjectId() || null);
        const { scheme, options } = await loadClassificationLeafOptions(projectId);
        const selected = classificationSelect.value && options.some((item) => String(item.id) === String(classificationSelect.value))
          ? classificationSelect.value
          : null;
        setClassificationSelectOptions(
          classificationSelect,
          options,
          selected,
          options.length ? `-- ${getLastConfiguredClassificationAlias(scheme)} 선택 --` : "-- 선택 가능한 분류 없음 --",
          !options.length,
        );
        updateClassificationHint({ nodeId: selected, scheme, options, target: "detail" });
      });
    ...
    } else if (key === "classification_node_id") {
      input = document.createElement("select");
      input.id = "asset-detail-classification-node-id";
      const projectId = Number(_selectedAsset.period_id || getCtxProjectId() || 0) || null;
      const { scheme, options } = await loadClassificationLeafOptions(projectId);
      setClassificationSelectOptions(
        input,
        options,
    ...
    if (key === "classification_node_id") {
      const hint = document.createElement("p");
      hint.className = "modal-hint";
      hint.id = "asset-detail-classification-hint";
      ...
      input.addEventListener("change", () => {
        updateClassificationHint({
          nodeId: input.value || null,
          target: "detail",
        });
      });
      const actions = document.createElement("div");
      actions.className = "modal-actions modal-actions-compact";
      const btn = document.createElement("button");
      btn.type = "button";
      btn.className = "btn btn-secondary btn-sm";
      btn.textContent = "분류체계에서 추가";
...
async function openCreateModal() {
  ...
  setClassificationSelectOptions(
    document.getElementById("asset-classification-node-id"),
    [],
    null,
    "-- 분류체계 불러오는 중 --",
    true,
  ...
  updateAssetRoleSuggestionHint("역할명 추천은 자산명 기준으로 참고용 제안만 제공합니다. 자동 선택되지 않습니다.");
  updateClassificationHint({ target: "create" });
...
async function refreshAssetClassificationSelect(selectedId = null) {
  const select = document.getElementById("asset-classification-node-id");
  if (!select) return;
  const projectId = Number(document.getElementById("asset-period").value || getCtxProjectId() || 0) || null;
  try {
    const { scheme, options } = await loadClassificationLeafOptions(projectId);
    const currentSelected = selectedId ?? select.value ?? null;
    const validSelected = currentSelected && options.some((item) => String(item.id) === String(currentSelected)) ? currentSelected : null;
    setClassificationSelectOptions(
      select,
      options,
      validSelected,
    ...
    updateClassificationHint({ nodeId: validSelected, scheme, options, target: "create" });
  } catch (err) {
    setClassificationSelectOptions(select, [], null, "-- 분류체계 불러오기 실패 --", true);
    updateClassificationHint({ target: "create" });
    showToast(err.message, "error");
  }
}
...
async function applyCatalogClassificationSuggestion(nodeCode) {
  ...
  const projectId = Number(document.getElementById("asset-period").value || getCtxProjectId() || 0) || null;
  const { scheme, options } = await loadClassificationLeafOptions(projectId);
  const matched = options.find((item) => item.node_code === nodeCode);
  if (!matched) return;
  setClassificationSelectOptions(
    select,
    options,
  ...
  updateClassificationHint({ nodeId: matched.id, scheme, options, target: "create" });
}
...
function renderCatalogDropdown(items) {
  ...
  dd.textContent = "";
  _catalogSearchResults = items.filter((item) => item.asset_type_key);
...
function selectCatalogItem(item) {
  ...
  syncRoleNameSuggestion();
  if (item.classification_node_code) {
    applyCatalogClassificationSuggestion(item.classification_node_code).catch(() => {});
  }
  updateAssetSaveState();
...
async function openInlineCatalogForm() {
  ...
  await populateInlineClassificationSelect();
  await populateInlineAssetTypeSelect();
}

async function populateInlineClassificationSelect(selectedCode = "") {
  const select = document.getElementById("inline-classification-node-code");
  if (!select) return;
  const { options } = await loadClassificationLeafOptions(null);
  select.textContent = "";
  const empty = document.createElement("option");
  empty.value = "";
  empty.textContent = options.length ? "-- 최종 분류 선택 --" : "-- 선택 가능한 분류 없음 --";
  select.appendChild(empty);
  options.forEach((option) => {
    const opt = document.createElement("option");
    opt.value = option.node_code;
    opt.textContent = option.path_label;
    select.appendChild(opt);
  });
  select.value = selectedCode || "";
}

async function populateInlineAssetTypeSelect() {
  const kind = document.getElementById("inline-kind").value;
  const sel = document.getElementById("inline-asset-type");
  if (!sel) return;
...
async function saveInlineCatalog() {
  ...
  const classificationNodeCode = document.getElementById("inline-classification-node-code").value;
  const assetTypeKey = document.getElementById("inline-asset-type").value;
  if (!vendor || !name || !productType || !classificationNodeCode || !assetTypeKey) {
  ...
  try {
    const item = await apiFetch("/api/v1/product-catalog", {
      ...
      body: {
        ...
        classification_node_code: classificationNodeCode,
...
async function saveAsset() {
  ...
  const payload = {
    ...
    classification_node_id: document.getElementById("asset-classification-node-id").value
      ? Number(document.getElementById("asset-classification-node-id").value)
      : null,
...
document.getElementById("asset-classification-node-id").addEventListener("change", (event) => {
  updateClassificationHint({ nodeId: event.target.value || null, target: "create" });
});
...
document.getElementById("inline-kind").addEventListener("change", populateInlineAssetTypeSelect);
