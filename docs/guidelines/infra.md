...
| 용어 | 설명 |
| --- | --- |
| 계약단위 (ContractPeriod) | common 모듈의 ContractPeriod. 인프라 작업의 기준 단위. customer_id NOT NULL |
...
| 제품 카탈로그 (ProductCatalog) | 글로벌 HW 제품 정보 (customer_id 없음). vendor+name 유니크 |
...
| 계약단위 업체 (PeriodCustomer) | 계약단위별 업체 역할(고객사/수행사/유지보수사/통신사/벤더) |
| 계약단위 담당자 (PeriodCustomerContact) | 계약단위별 담당자 역할(고객PM/수행PM/구축엔지니어 등) |
...
- **고객사 중심 구조**: `Asset`, `IpSubnet`, `PortMap`, `PolicyAssignment`는 `customer_id` FK로 고객사에 귀속된다. `ContractPeriod`는 `customer_id` NOT NULL로 고객사에 종속된다.
...
- `ProductCatalog`는 글로벌 리소스 (customer_id 없음). `HardwareSpec` 1:1, `HardwareInterface` 1:N 하위 리소스.
...
- `PeriodCustomer`로 계약단위-업체 역할(고객사/수행사/유지보수사/통신사/벤더)을 관리한다.
- `PeriodCustomerContact`로 계약단위-담당자 역할(고객PM/수행PM/구축엔지니어 등)을 관리한다. `PeriodCustomer`에 종속(CASCADE 삭제).
- Pin 고객사: `UserPreference`(key=`infra.pinned_customer_id`)로 사용자별 고정 고객사를 DB 저장. `infra.last_period_id`로 마지막 선택 계약단위 기억. topbar 2단 셀렉터(고객사+계약단위)로 컨텍스트 전환.
...
- Excel Import/Export는 고객사 단위로 수행한다. Export 시 옵션 계약단위 필터로 해당 계약단위 자산만 포함 가능. 3시트(Inventory/IP대역/Portmap) 구조.
- 제품 카탈로그 Import는 글로벌(customer_id 불필요). `spec` 도메인(제품+HW스펙)과 `eosl` 도메인(EOS/EOSL 날짜)으로 분리.
