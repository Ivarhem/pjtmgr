...
- **scope**: 고객사(`customer_id`) 단위. 라우터는 `customer_id`를 Form 파라미터로 수신.
...
- **중복 판정**: 자산은 `asset_name` 기준(고객사 범위), 서브넷은 `subnet` 기준, 포트맵은 항상 신규 생성
- **샘플 양식**: `GET /api/v1/infra-excel/template/{domain}`

### 제품 카탈로그 Import (글로벌)

- **scope**: 글로벌 (customer_id 불필요). 제품 카탈로그는 고객사에 종속되지 않음.
...
- **scope**: 고객사(`customer_id`) 단위. 옵션 `project_id`로 프로젝트 자산만 필터 가능.
- **API**: `GET /api/v1/infra-excel/export?customer_id=N&project_id=M`
- **시트 구성**: `01. Inventory`, `05. 네트워크 대역`, `03. Portmap`
- 프로젝트 필터 적용 시 Inventory 시트만 `ProjectAsset` N:M 기반 필터링. IP 대역/포트맵은 고객사 전체.
